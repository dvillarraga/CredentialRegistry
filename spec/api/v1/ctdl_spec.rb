    ...
    context 'success' do
        ...
        context 'with description sets' do
          ...
          context 'default params' do
            ...
            it 'returns query results with a total count and description sets' do
              post "/ctdl?include_description_sets=yes",
                   query.to_json,
                   'Authorization' => "Token #{auth_token}",
                   'Content-Type' => 'application/json'
          ...
          context 'custom params' do
            ...
            it 'returns query results with a total count and description sets' do
              post "/ctdl?include_description_sets=yes&include_description_set_resources=yes&per_branch_limit=10",
                   query.to_json,
                   'Authorization' => "Token #{auth_token}",
                   'Content-Type' => 'application/json'
      ...
      context 'with results metadata' do
        ...
        it 'returns query results with metadata' do
          expect {
            post "/ctdl?include_results_metadata=yes&log=no",
                 query.to_json,
                 'Authorization' => "Token #{auth_token}",
                 'Content-Type' => 'application/json'
          }.not_to change { QueryLog.count }
          ...
          expect_json('results_metadata.0.created_at', created_at1.as_json)
          expect_json('results_metadata.0.updated_at', updated_at1.as_json)
          expect_json('results_metadata.0.owned_by', owner1)
          expect_json('results_metadata.0.published_by', publisher1)
          expect_json('results_metadata.1.resource_uri', resource_uri2)
          expect_json('results_metadata.1.created_at', created_at2.as_json)
          expect_json('results_metadata.1.updated_at', updated_at2.as_json)
          expect_json('results_metadata.1.owned_by', owner2)
          expect_json('results_metadata.1.published_by', publisher2)
        end
      end
    end
  end
